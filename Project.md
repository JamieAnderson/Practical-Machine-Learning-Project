...
Before running the final C5.0 model on the Course's test set, I *estimated the test set error* three times.  The first was during the tuning process (10 fold cv).  The second was the average of the 100 bootstrap OOB samples, and the third was on the test set I created.  The errors (1-accuracy) were: *0.40%* , *0.35%*, and *1.1%*  respectively.<br>
...
modelfit+geom_point(position=position_jitter(w = 0.2),size=3,color="darkblue",alpha=0.5)+
        scale_y_continuous(limits=c(.65,1),breaks=seq(0.65,1,by=0.05))
```
...
```
According to my test set, C5.0 model has an accuracy of over 98.9%.  This is slightly lower, then the OOB estimate from the bootstrapping process.
